                ...
                <div class="flex flex-row gap-x-10">
                    ...
                    <div class="flex flex-col gap-y-10">    
                        <div>
                            ...
                            <h3 class="text-indigo-950 text-xl font-bold">{{ 'Rp. ' . number_format($subscribeTransaction->total_amount, 0, ',', '.') }}                            </h3>
                        </div>
                        <div>
                            <p class="text-slate-500 text-sm mb-2">Status</p>
                            @if ($subscribeTransaction->is_paid)
                            <span class="py-2 px-3 rounded-full bg-green-500 text-white text-sm font-bold">ACTIVE</span>
                            @else 
                            <span class="py-2 px-3 rounded-full bg-red-500 text-white text-sm font-bold">PENDING</span>
                            @endif
                        </div>
                        <div>
                            <p class="text-slate-500 text-sm">Checkout Date</p>
                            <h3 class="text-indigo-950 text-xl font-bold">{{ $subscribeTransaction->created_at->format('d F Y') }}</h3>
                        </div>
                        <div>
                            <p class="text-slate-500 text-sm">Subscription Start At</p>
                            <h3 class="text-indigo-950 text-xl font-bold">{{ \Carbon\Carbon::parse($subscribeTransaction->subscription_start_date)->translatedFormat('d F Y') }}
                            </h3>
                        </div>
                        <div class="">
                    ...
                    <div>
                        <div>
                            <img 
                                src="{{ Storage::url($subscribeTransaction->proof) }}" 
